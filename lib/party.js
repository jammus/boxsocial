...
Party.prototype.hasGuest = function(guest) {
    for (var x in this.guests) {
        if (this.guests[x].session.user.toLowerCase() === guest.session.user.toLowerCase() || (guest.session.key && this.guests[x].session.key.toLowerCase() === guest.session.key.toLowerCase())) {
            return true;
        }
    }
    return false
